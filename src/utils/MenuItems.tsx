export const MENU_DATA: any = {
  Beverages: [
    {
      title: "MOCKTAILS",
      icon: "/images/menuItems/Death By Chocolate.png",
      varients: [
        {
          title: "Cucumber Lime ",
          price: 115,
        },
        {
          title: "Virgin Mojito",
          price: 125,
        },
        {
          title: "Watermelon mojito",
          price: 125,
        },
        {
          title: "Minty mango",
          price: 125,
        },
        {
          title: "Blue Ocean ",
          price: 135,
        },
        {
          title: "Blue Berry Bash",
          price: 135,
        },
        {
          title: "Sunset glory",
          price: 135,
        },
        {
          title: "Pina Colada",
          price: 145,
        },
      ],
    },

    {
      title: "SHAKES",
      varients: [
        {
          title: "Vanilla Shake",
          price: 115,
        },
        {
          title: "Chocolate Shake",
          price: 125,
        },
        {
          title: "Banana Carmel Shake",
          price: 135,
        },
        {
          title: "Oreo Nutella Shake",
          price: 135,
        },
        {
          title: "Lotus Biscoff Shake",
          price: 135,
        },
        {
          title: "Mixed Berry Shake ",
          price: 145,
        },
        {
          title: "Death By Chocolate",
          price: 145,
        },
      ],
    },

    {
      title: "COFFEE",
      varients: [
        {
          title: "Classic Hot Coffee (Hot)",
          price: 75,
        },
        {
          title: "Caramel Coffee (Hot)",
          price: 85,
        },
        {
          title: "Hazelnut Coffee (Hot)",
          price: 85,
        },
        {
          title: "Classic Cold Coffee",
          price: 125,
        },
        {
          title: "Caramel Cold Coffee",
          price: 135,
        },
        {
          title: "Hazelnut Cold Coffee",
          price: 135,
        },
      ],
    },

    {
      title: "TEA",
      varients: [
        {
          title: "Green Tea (Hot)",
          price: 45,
        },
        {
          title: "Masala Tea (Hot)",
          price: 65,
        },
        {
          title: "English Breakfast Tea (Hot)",
          price: 65,
        },
        {
          title: "Lemon Iced Tea",
          price: 115,
        },
        {
          title: "Peach Iced Tea",
          price: 125,
        },
        {
          title: "Blueberry Iced Tea",
          price: 125,
        },
      ],
    },
  ],
  Pizza: [
    {
      title: "Veg PIZZA",
      icon: "/images/menuItems/pizza.png",
      varients: [
        {
          title: "Margherita Pizza",
          price: 185,
        },
        {
          title: "Garden Fresh",
          price: 215,
        },
        {
          title: "BBQ Panner ",
          price: 235,
        },
        {
          title: "Chipotle Panner",
          price: 235,
        },
      ],
    },

    {
      title: "Non-veg PIZZA",
      varients: [
        {
          title: "Chicken Carne",
          price: 265,
        },
        {
          title: "Chicken & Peppers",
          price: 275,
        },
      ],
    },
  ],
  Waffles: [
    {
      title: "WAFFLES",
      icon: "/images/menuItems/waffels.png",
      varients: [
        {
          title: "Mapple Syrup Waffle",
          price: 175,
        },
        {
          title: "Brownie Waffle",
          price: 185,
        },
        {
          title: "Nutellea & Banana Waffle",
          price: 205,
        },
        {
          title: "Triple Chocolate Waffle",
          price: 215,
        },
        {
          title: "Add on Ice Cream ",
          price: 25,
        },
      ],
    },
  ],
  Sandwich: [
    {
      title: "Veg SANDWHICH",
      icon: "/images/menuItems/sandwich.png",
      varients: [
        {
          title: "Classic Veg Sandwhich",
          price: 115,
        },
        {
          title: "Bombay Masala Sandwhich",
          price: 125,
        },
        {
          title: "Jalapeno Chesse Sandwhich",
          price: 135,
        },
        {
          title: "Paneer Tikka Sanwhich",
          price: 155,
        },
      ],
    },

    {
      title: "Non-veg SANDWHICH",
      varients: [
        {
          title: "Scramled Egg Sandwhich",
          price: 145,
        },
        {
          title: "Chicken Tikka Sandwhich",
          price: 165,
        },
      ],
    },
  ],
  Cheesecake: [
    {
      title: "CHESSECAKE",
      varients: [
        {
          title: "Caramel Chesse Cake",
          price: 225,
        },
        {
          title: "Stawberry Chesse Cake",
          price: 225,
        },
        {
          title: "Belgium Chocolate Chesse Cake",
          price: 235,
        },
        {
          title: "Lotus Biscoff Chesse Cake",
          price: 235,
        },
      ],
    },
  ],
  Pancakes: [
    {
      title: "Pancakes",
      varients: [
        {
          title: "Breakfast Panckae",
          price: 155,
        },
        {
          title: "Blueberries Pancake",
          price: 165,
        },
        {
          title: "Nutella Pancake",
          price: 175,
        },
        {
          title: "Lotus Biscoff Pancake",
          price: 185,
        },
      ],
    },
  ],
  Appetizer: [
    {
      title: "Veg Appetizer",
      icon: "/images/menuItems/fries.png",
      varients: [
        {
          title: "Chilli Garlic Pops",
          price: 95,
        },
        {
          title: "Veggie Fingers",
          price: 105,
        },
        {
          title: "Veg Nuggets",
          price: 115,
        },
        {
          title: "Chesse Coins",
          price: 125,
        },
      ],
    },

    {
      title: "Non-veg Appetizer",
      varients: [
        {
          title: "Chicken Popcorns",
          price: 125,
        },
        {
          title: "Chicken Nuggets",
          price: 125,
        },
        {
          title: "Chicken Strips",
          price: 245,
        },
        {
          title: "Chicken Box",
          price: 545,
        },
      ],
    },
  ],
  "Wraps and Fries": [
    {
      title: "Veg WRAPS",
      icon: "/images/menuItems/tortilla.png",

      varients: [
        {
          title: "Grilled Onion & Peppers",
          price: 135,
        },
        {
          title: "BBQ Paneer",
          price: 165,
        },
        {
          title: "Spicy Paneer",
          price: 165,
        },
      ],
    },

    {
      title: "Non-veg Wraps",
      varients: [
        {
          title: "BBQ Chicken",
          price: 185,
        },
        {
          title: "Peri-Peri Chicken",
          price: 185,
        },
      ],
    },

    {
      title: "Fries",
      varients: [
        {
          title: "Salted Fries",
          price: 95,
        },
        {
          title: "Peri-Peri Fries",
          price: 105,
        },
        {
          title: "Chesse Loaded Fries",
          price: 135,
        },
      ],
    },
  ],
  "Veg Burger & Paneer Burger": [
    {
      title: "Veg Burger",
      icon: "/images/menuItems/burger.png",
      varients: [
        {
          title: "Aloo Tikki",
          description:
            "Crispy Aloo Tikki Cutlet patty , Topped with Pickeles ,  Chessy Sauce , Tomatoes & Onions in a buttery bun",
          price: 35,
        },
        {
          title: "Potato Crunch",
          description:
            "Herb Patty , Chesse Sauce & Fresh Toppings nestled on a Butter Toasted Bun",
          price: 65,
        },
        {
          title: "Farmer's Favourite",
          description:
            "Classic Vegetable Patty , Onions , Lettuce , Olives, Cucumber Oil , all harmonized with OG's signautre sauces",
          price: 85,
        },
        {
          title: "Roadside",
          description:
            "Hand crafted Cilantro & Mint Chutney , Spied Patty , Onions , Tamarid Chutney , Cucumber , Tomatoes , tucked in a soft bun ",
          price: 95,
        },
        {
          title: "Jalapeno Kick",
          description:
            "Fresh Iceberg Lettuce, Tomatoes , Compliment Juicy Patties , with fresh onions,zesty jalapenos",
          price: 105,
        },
        {
          title: "Route 66",
          description:
            "Artisnal Bun dressed in chesse sauce , Jalapeno , Red Paprika , gourmet veg patty , raddish & Gherkins",
          price: 125,
        },
        {
          title: "Pineapple Crunch",
          description:
            "Savory fusion of a chesse and veg patty , a  succulent pineapple slice & onions , completed by OG's Signature sauces layered on a bun ",
          price: 135,
        },
        {
          title: "Wrath of Burger",
          description:
            "Spicy veg patty , Spicy sauces, Red paprika , Onions , Iceberg , jalapenos in a perfectly toasted bun ",
          price: 145,
        },
        {
          title: "Mushroom & Chesse Burger",
          description:
            "House - Buttered Bun graced with roasted mushrooms , gourment veg patty , Onions , fresh lettuce & a touch of OG's signature sauces",
          price: 155,
        },
        {
          title: "Fat Kid",
          description:
            "Two melting chesse patties stacked between house-buttered buns with spicy chesse sauces , zesty jalapenos , caramelized onions & cruncy lettuce ",
          price: 175,
        },
      ],
    },
    {
      title: "Paneer Buger ",
      varients: [
        {
          title: "Mint Paneer",
          description:
            "Toasted Buns Housing marinated Paneer , Tomatoes , onions , Lettuce & dizzeled with OG's  signature sauces ",
          price: 115,
        },
        {
          title: "Fiery Cottage Chesse",
          description:
            "Artisnal Buns dressed with OG's delectable spicy sauces , a Spicy paneer paty layoured by  Tomatoes Onions and Lettuce",
          price: 135,
        },
        {
          title: "Paneer Grizzler",
          description:
            "Toasted Buns , freshly grilled paneer patty , Onions ,Tomatoes , Iceberg Lettuce & OG's Signature Sauce",
          price: 145,
        },
        {
          title: "Peri-fect Paneer",
          description:
            "Peri-peri marinated paneer patty, topped with lettuce , tomatoes, onion & spicy sauces",
          price: 155,
        },
        {
          title: "Paneer Tikka",
          description:
            "House - Buttered buns , tikka flavoured paneer patty , onions , lettuce & handcrafted cilantro & mint chutney ",
          price: 165,
        },
      ],
    },
    {
      title: "Chicken Burger",
      varients: [
        {
          title: "Chicken Boom",
          description:
            "Minced  Chicken Cutlet patty , Topped with Pickles , Cheesy Sauce , Tomatoes & Onions in a buttery Bun",
          price: 75,
        },
        {
          title: "Classic Chicken Burger",
          description: "Signature Chicken [atty , Tomatoes,Lettuce",
          price: 85,
        },
        {
          title: "Hen - sational",
          Spicy: "(Mild Spicy)",
          description:
            "Grilled spicy Chicken Patty , spicy sauces , Onions , Tomatoes & Iceberg Lettuce",
          price: 105,
        },
        {
          title: "Low Rider",
          description: "Desi Chicken Patty , fried Onions , Lettuce & Sauces",
          price: 125,
        },
        {
          title: "The Hybrid",
          description: "Super Chicken Patty + Egg , Pickles , Cheesy Sauce",
          price: 155,
        },
        {
          title: "Chicken Grizzler",
          description:
            "Grilled Chicken Patty, Iceberg Lettuce, Onions & Tomatoes",
          price: 155,
        },
        {
          title: "Crispy Clucker",
          Spicy: " (Mild Spicy)",
          description: "Fried Chicken Patty,Gherkins/Lettuce& assorted Veggies",
          price: 165,
        },
        {
          title: "Chicken Tikka",
          Spicy: "( Mild Spicy)",
          description:
            "Chicken Tikka Patty, handcrafted Cilantro&Mint chutney , Lettuce & Onions",
          price: 165,
        },
        {
          title: "Fiery Clucker",
          Spicy: "(Very Spicy)",
          description:
            "Deep fried crispy chicken patty sprinkled with Peri-Peri, Lettuce, Onions& Sauces",
          price: 175,
        },
        {
          title: "Fire Bird",
          Spicy: "(Very Spicy)",
          description:
            "Chicken marinated & cooked in Peri-Peri Spices/ Lettuce & Onions",
          price: 175,
        },
      ],
    },
    {
      title: "EGG BURGER",
      varients: [
        {
          title: "Breakfast King",
          description:
            "Featuring a Sunny Egg Patty , sauteed Onions & ich Makhani Sauce",
          price: 65,
        },
        {
          title: "Peri-fect Sunrise (Mild Spicy)",
          Spicy: "(Mild Spicy)",
          description:
            "Peri Peri spiced double Egg Patty / Lettuce & crispy onions",
          price: 115,
        },
        {
          title: "Jalapeno Kick Sunrise",
          description: "Doiuble Egg Patty, briny Jalapeno/ Lettuce & Onions",
          price: 125,
        },
      ],
    },
  ],
  Pastas: [
    {
      title: "Veg Pastas",
      icon: "/images/menuItems/pasta.png",
      varients: [
        {
          title: "Alfredo Veg",
          description:
            "White sauce pasta tossed with Bell Peppers and Zucchini ",
          price: 235,
        },
        {
          title: "Arrabbiata Veg",
          description:
            "Spicy Pomodoro (Red Sauce) , Dried Redd Chilli Peppers cooked in Olive Oil",
          price: 235,
        },
        {
          title: "Mamma Rosa Veg",
          description:
            "Mixed sauce pasta with Broccoli, Zucchini and Bell Peppers",
          price: 235,
        },
        {
          title: "Pomodoro Veg",
          description:
            "Pasta tossed with Fresh Tomatoes, Olive oil , basil and Italian Herbs",
          price: 235,
        },
      ],
    },
    {
      title: "Non-veg Pastas",
      varients: [
        {
          title: "Alfredo Non-Veg",
          description:
            "White sauce pasta tossed with Bell Peppers and Zucchini ",
          price: 235,
        },
        {
          title: "Arrabbiata Non-Veg",
          description:
            "Spicy Pomodoro (Red Sauce) , Dried Redd Chilli Peppers cooked in Olive Oil",
          price: 235,
        },
        {
          title: "Mamma Rosa Non-Veg",
          description:
            "Mixed sauce pasta with Broccoli, Zucchini and Bell Peppers",
          price: 235,
        },
        {
          title: "Pomodoro Non-Veg",
          description:
            "Pasta tossed with Fresh Tomatoes, Olive oil , basil and Italian Herbs",
          price: 235,
        },
      ],
    },
  ],
};
